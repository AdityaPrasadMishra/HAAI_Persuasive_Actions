...
class SearchNode:
    ...
    def get_learned_model(self):
        model = Sequential()
        model.add(LSTM(128,input_shape=(26, 40), return_sequences=True))
    ...
    def get_explicability_score(self, act):
        ...
        maxvalwithss = 26
        nooffeatures = 40
        vocabdict= {'BLANK': 4, 'EXP': 3, 'START': 1, 'STOP': 2, 'UNEXP': 0}
        revevocabdict = {}
        for it in vocabdict:
            revevocabdict[vocabdict[it]] = it
        #print(revevocabdict)
        model = self.get_learned_model()
        model.load_weights("Sequential_Labels_DNNLSTM_55_LARGE.h5")
