...
class LinearFuncApprox:
    ...
    def update(self, state, explanation_features, action, val):
        '''
        Args:
            state (State)
            action (str)
        ...
        '''
        self._update_weights(state,explanation_features, action, val)

    def _phi(self, state, explanation_features, action):
        '''
        Args:
        ...
        basis_feats = np.append(state.features(), explanation_features)
        #print ("basis_feats",basis_feats, state.features(), explanation_features)
        if self.rbf:
    ...
    def _update_weights(self, cur_state, explanation_features, action, val):
        '''
        Args:
            reward (float)
            cur_state (State)
        ...
        max_q_cur_state = self.get_q_value(cur_state, explanation_features, action)
        #prev_q_val = self.get_q_value(self.prev_state, self.prev_action)
        self.most_recent_loss = val - max_q_cur_state 

        # Update each weight
        phi = self._phi(cur_state, explanation_features, action)
    ...
    def get_q_value(self, state, explanation_features, action):
        '''
        Args:
        ...
        sa_feats = self._phi(state, explanation_features, action)
