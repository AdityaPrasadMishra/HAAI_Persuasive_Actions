...
def get_best_action(state, model, acts, explanation_features):
    ...
    curr_best_act_val = model.predict(np_s)[0]
    # Equality check
    equal_flag = True
    for curr_index in range(len(acts)):
        np_s = np.array([list(state.features())+explanation_features+[curr_index]])
        curr_val =  model.predict(np_s)[0]
...
def tamer_algorithm():
    ...
    current_act_ind = epsilon_greedy(current_state, reg, all_actions, explanation_features)
    ...
    for i in range(1000):
        ...
        curr_state_feat = list(current_state.features()) + explanation_features + [current_act_ind]
        np_s = np.array([curr_state_feat])
        # We assume that the human model is optimal
        #if h != 0:
            # Online learning:
        print(current_state,reg.predict(np_s), h, all_actions[current_act_ind])
        ...
        current_act_ind = epsilon_greedy(new_state, reg, all_actions, explanation_features)
    ...
    print ("Best action from tamer",all_actions[get_best_action(current_state, reg, all_actions, explanation_features)])
