...
class PUDDLER:
    def __init__(self):
        ...
        self.base_agent = LinearQAgent(actions=self.base_human_model.get_actions(),num_features=2)
        run_single_agent_on_mdp(self.base_agent, self.base_human_model, episodes=10000, steps=4)
...
#            self.current_agent = self.base_agent
    ...
    def get_next_state(self,state,act):
        ...
        reward, state = self.base_human_model.execute_agent_action(act)
        return state
